...
public class PostController {
     ...
     * raw JSON
      {
        ...
        1) JWT 토큰 검증
        2) Security Context 에 인증 정보 저장
        3) Security Context 에서 인증 정보 추출
        4) 작성자 정보와 함께 게시글 저장
     */
    @PostMapping
    public ResponseEntity<?> createPost(@RequestBody PostCreateRequestDto requestDto){
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body(ApiResponseDto.success(postDetailResponseDto,
                "게시글이 작성되었습니다"));
    }
    /**
     * 게시글 상세 조회
    ...
    public ResponseEntity<?> getPostById(@PathVariable Long id){
        ...
        log.info("게시물 조회 성공:title={}",post.getTitle());
