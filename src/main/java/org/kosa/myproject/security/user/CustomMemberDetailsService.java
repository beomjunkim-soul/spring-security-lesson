...
/**
 CustomMemberDetailsService -> Spring Security 의 UserDetailsService의 구현체
 역할 : SpringFilterProxy에서 사용자 인증 (로그인 체크) 시 DB와 연동해 인증 여부를 담당
 */
...
public class CustomMemberDetailsService implements UserDetailsService {
     ...
     * 3. Repository 를 이용해 사용자 정보조회 후 UserDetails(사용자 정보) 를 반환
     * 4. Spring Security Filter Proxy 가 입력한 비밀번호와 UserDetails 비밀번호를 비교
     * 5. 일치하면 인증 성공, Authentication 객체를 생성 (SecurityContext 에 저장되는 정보 )
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        // 주의점 : findById 가 아니고 findByUsername 메서드로 호출
